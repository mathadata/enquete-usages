...
**75 paires** site ↔ Capytale (26,8 % des 280 cliqueurs site à UAI connu) : **53 de confiance A** (timing clic→clone serré, ou déploiement direct corroboré par l'activité), **22 de confiance B** (unicité UAI 1:1). Sur ces 75 : **47 s'étaient testés, 28 ont déployé sans test** (« plongée directe »), **61 (81 %) ont atteint une classe** (1 713 élèves), 33 ont réutilisé (≥2 séances), 22 ont diffusé ≥2 activités, 5 sur deux années. *(L'appariement récupère désormais les profs « plongée directe » sans clone-test — signaux déploiement D + activité E ; cf. correctif matcher.)*
...
- **Aucun identifiant commun.** Appariement individuel inféré (75 paires, dont 53 confiance A) ; grain robuste = établissement + cohorte. 13/75 paires ont une causalité inversée (clone avant clic site).
