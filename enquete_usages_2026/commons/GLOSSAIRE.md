...
>
> Implémenté par [`commons/build_profiles.py`](build_profiles.py) (couche de calcul
...
- **Type de formation** (`fcat`) : `presentiel` / `webinaire` (webdecouv + webinaire) / `nouveau`
  ...
  réels INCONNUS**, à ne **pas** compter comme webinaire). *(Détail dans `volet2/DEFINITIONS_VOLET2.md`.)*
...
- **2026-06-23** — Refonte canonique : (1) seuil classe **≥ 10 → ≥ 5** + bande **sous-seuil 1-4**
  ...
  séparément. Implémentation : `commons/build_profiles.py`.
