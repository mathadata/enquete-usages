...
### T186 — le power-user, et la plus nette trajectoire « fait, pas né »
Lille, lycée. Le prof le plus actif du corpus sur **toutes** les dimensions de largeur à la fois. **An 1** (mai-juin 2024) : un essai prudent — 6 micro-séances, 2 activités, classe max 3, cinq séances à 0 min (test sur soi). **An 2** : la bascule — 29 séances, 5 activités, vrais déploiements classe entière (16, 34, 36, 41 élèves), le catalogue grimpe vers Challenge MNIST et Droite & produit scalaire. **An 3** : consolidation — 13 séances, ouverture sur une Géométrie repérée à 39 élèves (73 min) + une activité neuve. **Bilan** : 404 élèves uniques (#1/224 ; **6,8 % de tous les élèves de la plateforme**), 48 séances (#1), **7 activités** (le seul ≥7 ; médiane = 1), 3 années, 101 élèves en travail-maison (le seul ≥100). La continuation atteint 10 % (~3× la médiane). *(n=1, descriptif ; ~14 séances à 0 min sont du test enseignant, pas des classes.)*

### T005 & T012 — le paradoxe « atteint puis disparu »
**T005** (Toulouse) : 3 séances, **toutes la même activité** « Intro IA » — lun. 19 mai 16 h (19 élèves, 52 min), un fragment 2 élèves le même jour, ven. 23 mai 10 h (**41 élèves**, 28 min). 42 élèves sur 4 jours, une activité, **jamais revenu**. **T012** (Pantin) : un one-shot plus riche — lun. 31 mars 14 h, **36 élèves**, 92 min, continuation 28 % — un vrai cours complet, fait une fois, **jamais répété**. **Le motif d'archétype** : les **41 déployeurs éligibles décrochent tous (0/41)**, chacun n'a utilisé **qu'une seule activité** — contre **90 % de retour (27/30)** chez ceux qui ont atteint une classe entière *avec* 2+ activités et ~5 séances. **La portée n'est pas le tueur ; la portée *peu profonde* l'est.** (Fisher p≈2e-11 sur le contraste classe-entière.)

### T188 — le pionnier durable : du travail autonome, pas un cours hebdo
Terminale. Corrige l'étiquette « petit groupe hebdomadaire ». Une activité dominante (« Droite & produit scalaire », 43 séances sur 47) utilisée en **travail individuel autonome**, réutilisée sur 2 années (avr. 2025 et avr. 2026). **27 des 47 séances sont à 1 élève**, réparties sur les 7 jours et toutes les heures — la signature d'élèves revenant seuls sur une activité-ancre. Continuation 53 %, 46 élèves en reprise, revenu. (Jumeau T190 : 47 séances, 475 j, continuation 84 %, 106 reprises.)
...
- **Petits effectifs** : soutien (n=14), demi-groupe (n=10, sous le seuil), fidèle (n=13) — **indicatifs**. Cas individuels = n=1, descriptifs, anonymes (pseudonymes ; commune/académie = grain établissement).
- **Reproductibilité** : `commons/build_scenarios.py` (table séances) → `commons/workflow_scenarios.js` (14 questions + vérif) → charts. Faits : `commons/data/facts_scenarios.json`, `scenarios_teachers.csv`, `sessions_enriched.csv` (pseudonymisés, PII-free).

---

**Sources de vérité** : `commons/data/scenarios_teachers.csv`, `sessions_enriched.csv`, `facts_scenarios.json`. Graphiques : `commons/charts/sea_*.png`. Page web : `commons/dashboard_seances.html`.
